...
export const isQuestCompleted = async (questId) => {
  try {
    const response = await axios.get(`${API_URL}/quests/${questId}/is_completed/`, getAuthHeaders());
    return response.data.is_completed;
  } catch (error) {
    if (error.response && error.response.status === 401) {
      const newAccessToken = await refreshToken();
      if (newAccessToken) {
        return await axios.get(`${API_URL}/quests/${questId}/is_completed/`, getAuthHeaders()).then((response) => response.data.is_completed);
      }
    }
    throw error;
  }
